...
const website = 'https://www.adidas.com/us/order-tracker';
const submitButton = '.gl-cta.gl-cta--primary.order-tracker__submit___2oWVr';
let outputRecords = [];
...
async function processTask(task) {
    ...
    console.log("checking: " + orderNumber);
...
async function main() {
    ...
    while (tasks.length > 0) {
        const taskBatch = tasks.splice(0, 5);
        await Promise.all(taskBatch.map(task => processTask(task).catch(error => {
